...
def tb_generate(chart_type,keyword_counts):
    ...
    if chart_type == "折线图":
        ...
        plt.boxplot(keyword_counts.values())
        plt.xlabel("关键字")
        plt.ylabel("数量")
        plt.xticks([1], ["Count"])
        plt.title("Tag Text Count Box Chart")
        st.pyplot(fig)
    elif chart_type=="区域图":
        ...
        x = range(len(keyword_counts.keys()))
        ...
        ax.set_xticklabels(keyword_counts.keys(), rotation="vertical")
        plt.title("Tag Text Count 3D Scatter Chart")
        st.pyplot(fig)
    elif chart_type=="热力图":
...
def ciyun():
    ...
    # 绘制词云
    word_cloud = plot_word_cloud(word_count, shape=selected_shape)
    # 保存词云为 HTML 文件
...
def data_analysis():
    ...
    url = st.text_input("输入网址", "https://www.gov.cn/xinwen/2022-10/25/content_5721685.htm")
    # 添加下拉框，让用户选择图表类型
    chart_type = st.selectbox("Select Chart Type", ["折线图", "饼图", "柱状图", "散点图", "直方图", "箱线图", "区域图", "3D散点图", "热力图"])
